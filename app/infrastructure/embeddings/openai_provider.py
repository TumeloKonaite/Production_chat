    ...
    def _request_embeddings(self, *, inputs: list[str]) -> dict[str, Any]:
        ...
        try:
            ...
            raise EmbeddingConfigurationError(
                f"Unable to fetch embeddings from OpenAI for model {self.model_name}."
            ) from exc
        except ValueError as exc:
            raise EmbeddingConfigurationError(
                f"Received an invalid embedding response from OpenAI for model {self.model_name}."
            ) from exc
    ...
    def _extract_embeddings(
        ...
        *,
        ...
        if not isinstance(raw_data, list) or len(raw_data) != expected_count:
            raise EmbeddingConfigurationError(
                "OpenAI returned an unexpected number of embeddings for "
        ...
        for row in ordered_rows:
            ...
            if not isinstance(embedding, list) or not all(
                ...
                raise EmbeddingConfigurationError(
                    f"OpenAI returned an invalid embedding payload for {self.model_name}."
                )
            vectors.append([float(value) for value in embedding])

        if len(vectors) != expected_count:
            raise EmbeddingConfigurationError(
                "OpenAI embedding response could not be normalized for "
